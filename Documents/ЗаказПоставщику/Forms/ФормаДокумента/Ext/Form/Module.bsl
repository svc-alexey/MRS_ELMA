...
//<< Портал-Юг, Кондрашихин, 2024.08.03 [9268, Обновление ERP 2.5.17]
//
&НаКлиенте
Перем КэшЦенТендера;
//>> Портал-Юг, Кондрашихин, 2024.08.03
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//<< Портал-Юг, Кондрашихин, 2024.08.03 [9268, Обновление ERP 2.5.17]
	// Вызов кода расширения MRIYA
	ПЛ_ФормыКлиент.ДокументЗаказПоставщикуФормаДокумента_ПередЗаписью(Объект, Отказ, ПараметрыЗаписи);
	//>> Портал-Юг, Кондрашихин, 2024.08.03
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//<< Портал-Юг, Кондрашихин, 2024.08.03 [9268, Обновление ERP 2.5.17]
	// функциональность расширение its тендеры
	
	ДокументЗаказПоставщику_MRS_ПослеЗаписи(ПараметрыЗаписи);
	//>> Портал-Юг, Кондрашихин, 2024.08.03
...
//
&НаКлиенте
Процедура ДокументЗаказПоставщику_MRS_ПослеЗаписи(ПараметрыЗаписи)
	//>>Швецов. 19.09.2023. Оповещение по Заказу из тендера
	Если MRS_Сервер.РольДоступнаВызовСервера("ПЛ_CostControlРучные") Тогда
	Иначе	
		Если Объект.MRS_ОтложенноеСогласование	Тогда
			ПоказатьПредупреждение(,"Данный заказ будет согласован CostControl автоматически",,"Согласование CostControl");		
		Иначе
			ПоказатьПредупреждение(,"Данный заказ НЕ будет согласован CostControl автоматически",,"Согласование CostControl");	
		КонецЕсли;
	КонецЕсли;
	//<<Швецов. 19.09.2023. Оповещение по Заказу из тендера
КонецПроцедуры //>> Портал-Юг, Кондрашихин, 2024.08.03
...
Процедура ПартнерПриИзмененииСервер()
	...
	// << MRS #ERP-5699 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 11.03.26, (Отключение заполнения типового)
	//ОтветственныеЛицаСервер.ЗаполнитьМенеджера(ДокументЗакупки);
	// >> MRS #ERP-5699 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 11.03.26, ()
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	ТекстСообщения  = СтрШаблон(НСтр("ru = 'Количество проведенных замен: %1';
									|en = 'Количество проведенных замен: %1'"), КоличествоПроизведенныхЗамен);
...
//<< Портал-Юг, Кондрашихин, 2024.08.03 [9268, Обновление ERP 2.5.17]
#Область РасширениеITSТендеры

//<< Портал-Юг, Кондрашихин, 2024.08.03 [9268, Обновление ERP 2.5.17]
//
&НаКлиенте
Процедура ITS_ТоварыПриИзмененииПосле(Элемент) 
	ПолеВидимость = Ложь;
	Номенклатура = Новый Массив;
	Для каждого ТекСтр из Объект.Товары цикл                                 
		Если ЗначениеЗаполнено(ТекСтр.Номенклатура) тогда                    
			Номенклатура.Добавить(ТекСтр.Номенклатура);
			ITS_ЦенаТендера = КэшЦенТендера.Получить(ТекСтр.Номенклатура);
			Если ITS_ЦенаТендера = Неопределено тогда				
				ЗаполнитьКэшЦен(КэшЦенТендера, Номенклатура, Объект.Партнер, Объект.Организация);	   
				ITS_ЦенаТендера = КэшЦенТендера.Получить(ТекСтр.Номенклатура);	
			КонецЕсли;
			ТекСтр.ITS_ЦенаТендера = ITS_ЦенаТендера;
			Если ITS_ЦенаТендера <> 0 Тогда
				ПолеВидимость = Истина;				
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	Если ПолеВидимость Тогда
		MRS_ДобавитьПолеЦенаТендераПриОкончании();
	КонецЕсли;
	
КонецПроцедуры //>> Портал-Юг, Кондрашихин, 2024.08.03 

Процедура MRS_ДобавитьПолеЦенаТендераПриОкончании()
           
    // Проверяем, существует ли уже колонка, чтобы не добавлять повторно
    Если ЭтаФорма.Элементы.Найти("ITS_ЦенаТендера") <> Неопределено Тогда
        
        ЭлементДатаРозлива = ЭтаФорма.Элементы.Найти("ITS_ЦенаТендера");
        
        ЭлементДатаРозлива.Видимость = Истина;
...
&НаСервереБезКонтекста
Процедура ЗаполнитьКэшЦен(Кэш, Номенклатура, Поставщик, Организация)

	Запрос = Новый Запрос;

	Запрос.Текст =
	"ВЫБРАТЬ
	|	MRS_СлужебныйТендерыОрганизации.Ссылка.МестоПроведенияТендера КАК МестоПроведенияТендера,
	|	MRS_СлужебныйТендерыОрганизации.Ссылка.ВидПроведенияТендера КАК ВидПроведенияТендера,
	|	MRS_СлужебныйТендерыОрганизации.Ссылка.ВидЦены КАК ВидЦены
	|ПОМЕСТИТЬ ВТ_ДоступныеПараметры
	|ИЗ
	|	Справочник.MRS_СлужебныйТендеры.Организации КАК MRS_СлужебныйТендерыОрганизации
	|ГДЕ
	|	MRS_СлужебныйТендерыОрганизации.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ITS_ТоварыПоставщиковПоТендерам.Номенклатура КАК Номенклатура,
	|	ITS_ТоварыПоставщиковПоТендерам.Цена КАК Цена
	|ИЗ
	|	РегистрНакопления.ITS_ТоварыПоставщиковПоТендерам КАК ITS_ТоварыПоставщиковПоТендерам
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДоступныеПараметры КАК ВТ_ДоступныеПараметры
	|		ПО ITS_ТоварыПоставщиковПоТендерам.ВидТендернойПроцедуры = ВТ_ДоступныеПараметры.ВидПроведенияТендера
	|			И ITS_ТоварыПоставщиковПоТендерам.МестоПроведения = ВТ_ДоступныеПараметры.МестоПроведенияТендера
	|ГДЕ
	|	ITS_ТоварыПоставщиковПоТендерам.Тендер <> ЗНАЧЕНИЕ(Документ.ITS_ТендерныйЛист.ПустаяСсылка)
	|	И ITS_ТоварыПоставщиковПоТендерам.Поставщик = &Поставщик
	|	И ITS_ТоварыПоставщиковПоТендерам.Регистратор.ВыигранныйТендер
	|	И ITS_ТоварыПоставщиковПоТендерам.Номенклатура В(&Номенклатура)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДоступныеПараметры КАК ВТ_ДоступныеПараметры
	|		ПО ЦеныНоменклатуры.ВидЦены = ВТ_ДоступныеПараметры.ВидЦены
	|ГДЕ
	...
	ЦеныТендера = ПакетРезультатов[ПакетРезультатов.Количество() - 2].Выбрать();
	СредниеЦены = ПакетРезультатов[ПакетРезультатов.Количество() - 1].Выбрать();
	
	Если ЦеныТендера.Количество() <> 0 Тогда
		
		Пока ЦеныТендера.Следующий() Цикл
			Кэш.Вставить(ЦеныТендера.Номенклатура, ЦеныТендера.Цена);
		КонецЦикла;	
		
	Иначе
		Пока СредниеЦены.Следующий() Цикл
			Кэш.Вставить(СредниеЦены.Номенклатура, СредниеЦены.Цена);
		КонецЦикла;	
	КонецЕсли;
...
&НаКлиенте
Процедура MRS_ОтправитьЗаказПоставщикуПосле(Команда)
	
	//<< Туртанов А.А. 14.11.2023  № Заявки:"ERP-4415" "Автоматизировать заказы поставщикам"
	//Если НЕ Объект.ПЛ_ПодтвержденоCostControl Тогда		
	//	Сообщение = Новый СообщениеПользователю;
	//	Сообщение.Поле = "Объект.ПЛ_ПодтвержденоCostControl";
	//	Сообщение.Текст = "Данный документ не подтвержден CostControl!";
	//	Сообщение.Сообщить();
	//	Возврат;
	//КонецЕсли;
	
	СтруктураИсточника = Новый Структура;
	СтруктураИсточника.Вставить("Ссылка", Объект.Ссылка);
	
	ОповещениеПослеЗакрытияВопросаОбщегоЗаполнения = Новый ОписаниеОповещения("MRS_ПослеЗакрытияВопроса", ЭтотОбъект, СтруктураИсточника); 
	ТекстВопроса = "Поставщику " + "«" + Объект.Контрагент + "»" + " будет отправлена печатная форма «Заказ поставщику с желаемой датой поставки», продолжить?";
	ПоказатьВопрос(ОповещениеПослеЗакрытияВопросаОбщегоЗаполнения, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	//>> Туртанов А.А.
	
КонецПроцедуры

&НаКлиенте	
Процедура MRS_ПослеЗакрытияВопроса(Результат, СтруктураИсточника) Экспорт 
	...
	MRS_ВыполнитьКомандуВнешнейОбработки(СтруктураИсточника);
	//>> Туртанов А.А.
	
КонецПроцедуры

&НаКлиенте	
Процедура MRS_ВыполнитьКомандуВнешнейОбработки(СтруктураИсточника) Экспорт
	
	//<< Туртанов А.А. 14.11.2023  № Заявки:"ERP-4415" "Автоматизировать заказы поставщикам"
	СсылкаНаОбработку = MRS_ПолучитьСсылкуНаВнешнийОтчетОбработкуПоИмениНаСервере("Отправить печатную форму ""Заказ поставщику с желаемой датой поставки""");
	ИмяОбработкиСлужебное = ДополнительныеОтчетыИОбработкиВызовСервера.ПодключитьВнешнююОбработку(СсылкаНаОбработку);
	
	ОткрытьФорму("ВнешняяОбработка." + ИмяОбработкиСлужебное + ".Форма", СтруктураИсточника);
	//>> Туртанов А.А.
	
КонецПроцедуры 

&НаСервере
Функция MRS_ПолучитьСсылкуНаВнешнийОтчетОбработкуПоИмениНаСервере(ИмяОтчетаОбработки) Экспорт 
...
КэшЦенТендера = Новый Соответствие;
#КонецОбласти
//>> Портал-Юг, Кондрашихин, 2024.08.03