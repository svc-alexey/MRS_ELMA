...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(Договор) И Договор.Организация.ГоловнаяОрганизация <> Организация.ГоловнаяОрганизация Тогда
		ОбщегоНазначения.СообщитьПользователю("Запрещено проведение документа с организацией в договоре отличной от организации документа!",,,,Отказ);
	КонецЕсли;
	//<<
	...
	ITS_ТендерыСервер.ДокументЗаказПоставщикуПередЗаписью(ЭтотОбъект, Отказ, РежимЗаписи, РежимПроведения);
...
//<<Портал-Юг, Давиденко 2019.02.08 [3.2.8 Редактирование цены в обработке «формирование заказов по потребностям»]
		//		ЗаполнитьЦеныПоВидамЦен(); //заполнение цен, согласно видам цен, заданным в таблице "Товары".
		Если ДанныеЗаполнения.Товары.Колонки.Найти("Цена") <> Неопределено Тогда
			//вызов из обработки с установленной вручную ценой
			ЗаполнитьЦеныПоВидамЦен(Истина); //заполнение цен, согласно ценам, указанным в данных заполнения.
		//>>Портал-Юг, Давиденко 2019.02.08
		...
		//<<Портал-Юг, Давиденко 2019.02.08 [3.2.7 Уточнения из заказов на перемещение для закупок]
		Если ДанныеЗаполнения.Товары.Колонки.Найти("ПЛ_Заказ") <> Неопределено Тогда
			ПЛ_МассивЗаказов = ДанныеЗаполнения.Товары.ВыгрузитьКолонку("ПЛ_Заказ");
			ПЛ_МассивРезультат = ОбщегоНазначенияУТ.УдалитьПовторяющиесяЭлементыМассива(ПЛ_МассивЗаказов);
			Если ПЛ_МассивРезультат.Количество() = 1 Тогда
				ЭтотОбъект.ДокументОснование = ПЛ_МассивРезультат[0];
			КонецЕсли;
		КонецЕсли;
		//>>Портал-Юг, Давиденко 2019.02.08	
...
Процедура ЗаполнитьЦеныПоВидамЦен(ПЛ_ЦеныФиксированы = Ложь)
//>>Портал-Юг, Давиденко 2019.02.08
	...
	Пока Выборка.Следующий() Цикл  
		//<<Портал-Юг, Давиденко 2019.02.06 [3.2.8 Редактирование цены в обработке «формирование заказов по потребностям»]
		Если Не ПЛ_ЦеныФиксированы Тогда
		//>>Портал-Юг, Давиденко 2019.02.06		
...
Процедура ЗаполнитьДокументНаОснованииЗаказаНаПеремещение(Знач ДанныеЗаполнения)
	...
	ЗаказНаПеремещение = ДанныеЗаполнения.Основание;
		...
		|	Заказ.ЖелаемаяДатаПоступления КАК ЖелаемаяДатаПоступления
		|ИЗ
	...
	//>>Швецов. ERP-5057. 28.05.2025. Закрытие заказов по желаемой дате поступления
	ЖелаемаяДатаПоступления = Реквизиты.ЖелаемаяДатаПоступления;                   
	//<<Швецов. ERP-5057. 28.05.2025. Закрытие заказов по желаемой дате поступления
	
	//<< Портал-Юг, Кондрашихин, 2024.08.02 [9268, Обновление ERP 2.5.17]
	// вызов кода расширения "СкладПолучатель"
	ПЛ_Сервер.склпл_ЗаполнитьДокументНаОснованииЗаказаНаПеремещение(ДанныеЗаполнения, ЭтотОбъект);
	//>> Портал-Юг, Кондрашихин, 2024.08.02
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	//<<Портал Юг, Финик, 2021.15.03 SD39937
	 ЭтотОбъект.ПЛ_ПодтвержденоCostControl = Ложь;
	//>> Портал-Ю, Финик 2021.15.03 SD39937	
