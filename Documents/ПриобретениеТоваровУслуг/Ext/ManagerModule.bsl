...
Процедура ТекстЗапросаДанныеДокументаТаблицаТовары(СписокЗапросов)
	...
	|	ТаблицаТовары.алкСуммаАкциза КАК алкСуммаАкциза,
	//>РАП: алкИнтеграцияЕГАИС КТ-2000
...
Функция ТекстЗапросаВтТаблицаТовары(Запрос, ТекстыЗапроса, ДокументОбъект) Экспорт
	...
	//<РАП: алкИнтеграцияЕГАИС КТ-2000
	|	ВЫРАЗИТЬ(ТаблицаТовары.алкСуммаАкциза * &КоэффициентПересчетаВВалютуРегл  КАК ЧИСЛО(31,2)) КАК алкСуммаАкцизаРегл,
	|	ВЫРАЗИТЬ(ТаблицаТовары.алкСуммаАкциза * &КоэффициентПересчетаВВалютуУПР  КАК ЧИСЛО(31,2)) КАК алкСуммаАкциза,
	//>РАП: алкИнтеграцияЕГАИС КТ-2000
	...
	|	КОНЕЦ КАК НаправлениеДеятельности,
	//<РАП: алкИнтеграцияЕГАИС КТ-2000
	|	0 КАК алкСуммаАкцизаРегл,
	|	0 КАК алкСуммаАкциза,
	//>РАП: алкИнтеграцияЕГАИС КТ-2000
	|	ЕСТЬNULL(АналитикаРасчетов.КлючАналитики, Неопределено) КАК АналитикаУчетаПоПартнерам,
	|	ТаблицаТовары.АналитикаУчетаНоменклатурыНоменклатураТипНоменклатуры КАК АналитикаУчетаНоменклатурыНоменклатураТипНоменклатуры,
	|	ТаблицаТовары.АналитикаУчетаНоменклатурыНоменклатура КАК АналитикаУчетаНоменклатурыНоменклатура,
...
Процедура ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента) Экспорт
	...
	//<< Швецов, 2026.04.17 [Добавлена подсистема Тендеры]
	ITS_ТендерыСервер.ЗарегистрироватьУчетныеМеханизмы(МеханизмыДокумента);
	//>> 
...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		//<< Швецов, 2026.04.17 [Добавлена подсистема Тендеры]
		ITS_ТендерыСервер.ТекстЗапросаВременнаяТаблицаТоварыПоставщиковПоТендерам(Запрос, ТекстыЗапроса);
		//>> 
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	// ++ КТ-2000
	Если Метаданные.Документы.Найти("алкПечатьЭтикетокNiceLabel") <> Неопределено Тогда
		Документы.алкПечатьЭтикетокNiceLabel.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	КонецЕсли;
	// -- КТ-2000
	...
	Документы.лисЛабораторныйАнализ.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	// >> MRS #PROERP-11 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 08.06.26, (Разработка по ЧТЗ Приемка сырья)
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, СкладПоступления = Неопределено,
	...
	Запрос.УстановитьПараметр("Подразделение",             ДанныеОтбора.Подразделение);
		...
		//<< Портал-Юг, Свинцов И.И., 2019.08.26 [Интерфейс] 
	// Отключение отбора по подразделениям в "подборе товаров заказов" из приобретения товаров  
	...
	//>> Портал-Юг, Свинцов И.И., 2019.08.26	
    ...
    |       ВЫБОР когда
	|			ЗаказыОстатки.ЗаказПоставщику Ссылка Документ.ЗаказПоставщику тогда
	|               Выбор когда ЗаказыОстатки.ЗаказПоставщику.Статус = Значение(Перечисление.СтатусыЗаказовПоставщикам.Согласован)
	|				    тогда ЗаказыОстатки.ЗаказаноОстаток 
	|				иначе
