//<< Портал-Юг, Свинцов И.И., 2019.02.28 [5.2.30 Учёт остатков по маркам алкогольной продукции в разрезе складов] 
// Добавлено движение по РН "ПЛ_АлкогольнаяПродукцияПоМаркамНаСкладах"
//>> Портал-Юг, Свинцов И.И., 2019.02.28
//<< Портал-Юг, Баринов, 2019.09.12 [9.02 Контроль заполнения входящего номера и даты для документа «Приобретение товаров и услуг»]
// Для реквизитов НомерВходящегоДокумента и ДатаВходящегоДокумента свойство "Проверка заполнения" установлено в значение "Выдавать ошибку".
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// << MRS #ERP-5931 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 02.07.26, ()
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	//<< Портал-Юг, Гребенюк, 25.04.2023, [Обновление 2.5, Добавил тип из старого релиза проверить]
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда 
	ЦенаВключаетНДС = Истина;
	ЭтотОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика;
	//>> Портал-Юг, Гребенюк, 25.04.2023	
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(ОргСклада) И Организация <> ОргСклада Тогда
		ОбщегоНазначения.СообщитьПользователю("Запрещено проведение документа с организацией отличной от организации склада!",,,,Отказ);
	КонецЕсли;
	// >> MRS #ERP-4947 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 11.03.25, () 
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение И ЗначениеЗаполнено(Договор) И Договор.Организация.ГоловнаяОрганизация <> Организация.ГоловнаяОрганизация Тогда
		ОбщегоНазначения.СообщитьПользователю("Запрещено проведение документа с организацией в догоре отличной от организации документа!",,,,Отказ);
	КонецЕсли;
	//<<
...
Процедура ЗаполнитьДокументНаОснованииЗаказаПоставщику(Знач ДокументОснование, Знач СкладПоступления = Неопределено, РеквизитыЗаказа = Неопределено)
	...
	Если ТипОснования = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		//<<===Портал-Юг Крым === Плотникова Н.Ю. 25.12.2020 [36099]
		|				ИЛИ ЗаказПоставщику.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПоставщикам.Согласован)
		//>>===Портал-Юг Крым === Плотникова Н.Ю. 25.12.2020 [36099]	
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
...
Функция РазрешеноПробитиеФискальныхЧековПоДокументу()
