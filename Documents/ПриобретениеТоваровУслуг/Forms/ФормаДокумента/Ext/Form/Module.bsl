...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ОтчетаОбИмпортеПродукцииЕГАИС" 
		Или ИмяСобытия = "ИзменениеСостоянияЕГАИС"
		Или ИмяСобытия = "ВыполненОбменЕГАИС" Тогда 
		алкОбработкаОповещения(ИмяСобытия, Параметр, Источник);
	КонецЕсли;
	//>РАП: алкКТ2000Алкоголь КТ-2000
	
КонецПроцедуры
// КТ-2000*** КТ-2000*** Вставить область в конец модуля

#Область ПроцедурыИФункции_КТ_2000

&НаСервере
Процедура алкОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	
	Если ИмяСобытия = "Запись_ОтчетаОбИмпортеПродукцииЕГАИС"
		И Параметр.Основание = Объект.Ссылка Тогда
		
		алкИнтеграцияЕГАИСУТ.СформироватьТекстДокументаЕГАИС(ЭтаФорма);
...
КонецПроцедуры // алкОбработкаОповещения()

&НаКлиенте
Процедура алкТекстДокументаЕГАИСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ТекстДокументаЕГАИСОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если "СвязатьСТТНЕГАИС" = НавигационнаяСсылкаФорматированнойСтроки Тогда
		...
		ЭтотОбъект,,,,
		Новый ОписаниеОповещения("ОбработатьВыборТТНЕГАИСИзСписка", ЭтотОбъект));
	//<РАП: алкИнтеграцияЕГАИС КТ-2000
	ИначеЕсли "СоздатьОтчетОбИмпортеЕГАИС" = НавигационнаяСсылкаФорматированнойСтроки Тогда
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Основание", Объект.Ссылка);
		ОткрытьФорму("Документ.алкОтчетОбИмпортеПродукцииЕГАИС.Форма.ФормаДокумента", ПараметрыФормы, ЭтотОбъект);
	//<РАП: алкИнтеграцияЕГАИС КТ-2000
	КонецЕсли;
...
Процедура СформироватьТекстДокументаЕГАИС()
	...
	Иначе
		...
		КонецЕсли;
	//<РАП: алкИнтеграцияЕГАИС КТ-2000
	ЭтаФорма.Элементы.ТекстДокументаЕГАИС.УстановитьДействие("ОбработкаНавигационнойСсылки", "алкТекстДокументаЕГАИСОбработкаНавигационнойСсылки");
	алкИнтеграцияЕГАИСУТ.СформироватьТекстДокументаЕГАИС(ЭтаФорма);
	Строки.Добавить("  ");
	Строки.Добавить(ЭтаФорма.ТекстДокументаЕГАИС);
	//>РАП: алкИнтеграцияЕГАИС КТ-2000
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// << MRS #ERP-4947 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 25.02.25, ()
	УстановитьОрганизациюПоСкладу("Чтение");
	// >> MRS #ERP-4947 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 25.02.25, ()
	
	//>>Швецов. ERP-4995 07.04.2025 Проверка на договор не той организации
	ПроверитьДоговорПоОрганизации("Изменение");
	// <<
КонецПроцедуры
...
Процедура ГруппаСтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	// << MRS #PROERP-11 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 08.06.26, (Разработка по ЧТЗ Приемка сырья)
	Если ТекущаяСтраница.Имя = "ГруппаДополнительно" Тогда
		МРС_ДобавитьКнопкуЗапросКачестваВСДВПриобритении();
	КонецЕсли;
	// >> MRS #PROERP-11 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 08.06.26, (Разработка по ЧТЗ Приемка сырья)
...
Процедура ТоварыНоменклатураПартнераПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	//+++ КТ2000 Денис
	СтруктураДействий.Вставить("ЗаполнитьВидОперацииНалогообложенияАкцизом", Новый Структура("Номенклатура, Организация, ВидДокумента, СерияНоменклатуры, НалогообложениеНДС, ОбъектСсылка, ОбъектДата",
				ТекущаяСтрока.Номенклатура, Объект.Организация, "Приобретение товаров и услуг", ТекущаяСтрока.Серия, Объект.НалогообложениеНДС, Объект.Ссылка, Объект.Дата));
...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СтруктураДействий = Новый Структура;   
	// << MRS #ERP-5037 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 22.04.25, (Единица измерения для подстановки в документы)
	...
	ИначеЕсли ТекущаяСтрока.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Услуга") Тогда
		ТекущаяСтрока.СписатьНаРасходы = Истина;
		// << MRS #ERP-5696 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 24.02.26, ()
		ТекущаяСтрока.СтатьяРасходов = MRS_СлужебныеПараметрыПовтИсп.ПолучитьЗначение("СтатьяРасходовДляУслуг","Значение");
		ТекущаяСтрока.Подразделение = ОбщегоНазначенияУТВызовСервера.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(Объект.Склад, "Подразделение");
		// >> MRS #ERP-5696 ДенисламовАД <Adel.Denislamov@mriyaresort.com> 24.02.26, ()
	КонецЕсли;
...
// >> Швецов. ERP-5785. 06.04.26. Команды для интеграции с Диадок. Перенесено из расширения
...
Процедура СкладПриИзмененииСервер()
	...
	//<< Портал-Юг, Баринов, 2019.08.19 [5.2.29 Автоматическая генерация серий и заполнение документа «Приобретение товаров и услуг»]
	// При формировании по ТТН ЕГАИС склад может быть пустым, при заполнении склада, серии станут "неактуальны".
	// Дозаполним склад в сериях, только если он пустой, а в шапке не СкладГруппа 
	Если Не СкладГруппа И ЗначениеЗаполнено(Объект.Склад) Тогда
		Для каждого СтрокаСерии Из Объект.Серии Цикл
			Если Не ЗначениеЗаполнено(СтрокаСерии.Склад) Тогда
				СтрокаСерии.Склад = Объект.Склад;
			КонецЕсли; 
		КонецЦикла; 
	КонецЕсли; 
	//>> Портал-Юг, Баринов, 2019.08.19	
...
//>>Швецов. ERP-4995 07.04.2025 Проверка на договор не той организации
&НаСервере
Процедура ПроверитьДоговорПоОрганизации(ТекДействие)
	
	Если ЗначениеЗаполнено(Объект.Договор) Тогда

		ОрганизацияДоговора	= ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "Организация").ГоловнаяОрганизация;
		
		Если ОрганизацияДоговора <> Объект.Организация.ГоловнаяОрганизация И ТекДействие = "Изменение" Тогда
			
			ЭтаФорма.Модифицированность = Истина;
			ЗаполнитьДоговорПоУмолчанию();
			ОбщегоНазначения.СообщитьПользователю("В документе автоматически был изменен договор по умолчанию!");
			
		ИначеЕсли ОрганизацияДоговора <> Объект.Организация.ГоловнаяОрганизация И (Объект.Проведен ИЛИ ТекДействие = "Чтение") Тогда
			ОбщегоНазначения.СообщитьПользователю("ВНИМАНИЕ! В документе различается организация договора с организацией в документе!");
		КонецЕсли;
...
// << MRS #PROERP-11 ВикторовАГ <Artem.Viktorov@mriyaresort.com> 08.06.26, (Разработка по ЧТЗ Приемка сырья)
#Область МРС_Сыроварня

&НаСервере
Процедура МРС_ДобавитьКнопкуЗапросКачестваВСДВПриобритении()
	
	МРС_Сыроварня_Сервер.МРС_ДобавитьКнопкуЗапросКачестваВСДВПриобритении(ЭтотОбъект);	
	
КонецПроцедуры

&НаКлиенте
Процедура МРС_ЗапросКачестваВСД(ТекДействие)    	            
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		ПоказатьПредупреждение(, "Перед запросом необходимо записать документ!");
		Возврат;
	КонецЕсли;
	
	МРС_Сыроварня_Клиент.ОписаниеКачестваВСДДокументПриобретение(ЭтотОбъект);
